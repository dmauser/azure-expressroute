...
- [Lab steps:](#Lab-steps:)
    ...
    - [1) Deploy Azure side: Hub and Spoke](#1-deploy-azure-side-hub-and-spoke)
	- [2) Deploy OPNsense NVA](#2-deploy-opnsense-nva)
	- [3) Create both ExpressRoute Circuits](#3-create-both-expressroute-circuits)
	- [4) Create the UDRs](#4-create-the-udrs)
	- [5) Deploy GCP side (optional)](#5-deploy-gcp-side-optional)
	* [6) OPNSense NVA Configuration](#6-opnsense-nva-configuration)
	* [7) Validations](#7-validations)
	* [8) Misc/Troubleshooting](#8-misctroubleshooting)
- [Cleanup](#Cleanup)

## Intro

It deploys the following diagram using GCP as remote side for On-Premises and AVS (or another On-Premises). It requires that you have a Cloud Provider to interconnect between both GCP environments and Azure.

This script uses two OPNSense NVAs in HA mode. You can setup your own NVA device by skipping the NVA deployment section.

## Architecture diagram

![Diagram](./media/ars-nva-hairpin-lab.png)

## Lab steps

:point_right: **Note:** The commands below have been validated on Ubuntu bash console using az (Azure CLI) and gcloud (Google CLI) commands.

On the script listed below, make your changes based on your requirements on the variables section of each deployment (Azure or GCP).

### Prerequisite

Install az (Azure CLI) and gcloud (GCP CLI)
...
### 3) Create both ExpressRoute Circuits
...
### *** Provision ER with your Service Providers before next step ***
...
Those UDRs are used to send traffic to the NVA LB.
...
### 5) Deploy GCP side (optional)

Deploy both remote locations or branches environments. In the scope of this lab is AVS and On-premises.

#### AVS environment

```bash
# Define your variables <===== Make changes here based on your requirements =====>
...
```bash
#GCP
# Note use the same variables used to configure deploy GCP side listed above to delete each enviroment (run twice to each remote environment)
